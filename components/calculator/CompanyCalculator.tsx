...
import { TrendingUp, Scale, AlertCircle, Info } from 'lucide-react'
...
export function CompanyCalculator() {
    ...
    React.useEffect(() => {
        ...
        setResults(res);
    }, [totalIncome, bookProfit, isConcessional, isInitialPhase]);
                ...
                <Card className={`p-6 border-white/10 transition-all ${results.is_mat_applicable ? 'bg-blue-500/10 border-blue-500/30' : 'bg-background/40 opacity-70'}`}>
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-1.5">
                            <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground text-blue-400">MAT (115JB)</span>
                    ...
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-white/5">
                            <tr>
                                ...
                                <td className="py-3 text-right font-mono">{formatINR(results.effective_tax)}</td>
                            </tr>
                            <tr>
                                <td className="py-3 text-muted-foreground">Surcharge</td>
                                <td className="py-3 text-right font-mono">{formatINR(results.surcharge)}</td>
                            </tr>
                            <tr>
                                <td className="py-3 text-muted-foreground">Health & Education Cess (4%)</td>
                                <td className="py-3 text-right font-mono">{formatINR(results.cess)}</td>
                            </tr>
